...
def predict(net, img, out_img_path=None, device='cuda'):
    ...
    with torch.no_grad():
        ...
        mask_pred = (F.sigmoid(masks_pred) > 0.5).cpu().detach().numpy().astype(np.uint8).squeeze()
    if out_img_path is not None:
        masks_pred_im = Image.fromarray(mask_pred, mode='1')
        Path(out_img_path).parent.mkdir(parents=True, exist_ok=True)
        masks_pred_im.save(out_img_path)
    return mask_pred


def extract_aif_mask(in_img_path_or_seq, model_path_or_net="./models/best_model_ica_top.pt", device='cuda'):
    """PerfDSA"""
    '''Load the perfDSA segmentation model'''
    if isinstance(model_path_or_net, torch.nn.Module):
        logging.info(f'Using provided model')
        model = model_path_or_net
    else:
        model = torch.load(model_path_or_net, map_location=device)
        logging.info(f'Model loaded from {model_path_or_net}')

    '''Segmentation'''
    if isinstance(in_img_path_or_seq, np.ndarray):
        in_img = in_img_path_or_seq
        if len(in_img.shape) <= 3:
            # We have a sequence of 2D + t (t,x,y), but the model expects (b,t,x,y), so we have to unsqueeze
            in_img = np.expand_dims(in_img, axis=0)
    elif os.path.isfile(in_img_path_or_seq):
        in_img = load_image(in_img_path_or_seq, img_size=1024)
    else:
        ValueError("Input file not found: {}".format(in_img_path_or_seq))

    ica_top_seg_mask = predict(model, in_img)
    return ica_top_seg_mask
...
if __name__ == '__main__':
    ...
    '''Global settings'''
    args = get_args()
    extract_aif_mask(args.in_img_path, args.out_img_path)
    logging.info("Done!")
