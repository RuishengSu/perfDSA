...
def slices(slices_in,  # the 2D slices
    ...
    if return_vis:
        with tempfile.NamedTemporaryFile(suffix=".png", delete=True) as tmpfile:
            plt.savefig(tmpfile, format="png", dpi=dpi)  # File position is at the end of the file.
            plt.close()
            # tmpfile.seek(0)  # Rewind the file. (0: the beginning of the file)
            # return plt.imread(tmpfile)
            return cv2.imread(tmpfile.name, cv2.IMREAD_GRAYSCALE)
            # return iio.imread(tmpfile.name)
